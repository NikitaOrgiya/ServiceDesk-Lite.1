...
> и администратору (для всех заявок). Всё **выполнено и проверено везде, где
> это можно сделать без реального Neon-проекта**: unit-тесты, lint,
> typecheck, build — проходят; SQL-миграции и SQL security/functional-тесты
> выполнены и проверены на настоящем локальном PostgreSQL 16 (см.
> [«Что реально проверено»](#что-реально-проверено)). **Интеграционная
> приёмка на настоящем Neon Auth + Neon Data API не выполнялась** — ни в
> одной сессии разработки не было доступного Neon-проекта/credentials; это
> следующий шаг перед production-релизом (см. [«Ограничения»](#21-ограничения)).
...
Ни в одной сессии разработки (включая добавление административной панели,
комментариев/истории и это финальное portfolio-readiness исправление) не
было доступного Neon-проекта/credentials. Таблица ниже честно разделяет,
что и когда проверялось: строки `test`/`lint`/`typecheck`/`build` —
актуальный прогон после последнего изменения кода в этом репозитории;
SQL-модель (миграции, RLS, RPC, security/functional-тесты) — прогон на
настоящем локальном PostgreSQL 16 во время первоначальной миграции на Neon;
с тех пор ни RLS, ни RPC, ни миграции не менялись, поэтому тот результат
остаётся в силе и не был заново перепрогнан в последующих сессиях:
...
- Интеграционная приёмка на настоящем Neon (Auth, Data API, RLS с реальными
  JWT) не выполнялась — ни в одной сессии разработки не было доступного
  проекта/credentials. Это следующий шаг перед production-релизом.
