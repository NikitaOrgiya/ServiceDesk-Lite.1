...
--
-- Neon Data API roles are `anon` and `authenticated` — there is no
...
REVOKE ALL ON public.ticket_number_counters FROM PUBLIC, anon, authenticated;
REVOKE ALL ON public.tickets FROM PUBLIC, anon, authenticated;
REVOKE ALL ON public.ticket_comments FROM PUBLIC, anon, authenticated;
REVOKE ALL ON public.ticket_history FROM PUBLIC, anon, authenticated;

-- anon: no application table access at all.
