...
const { updateAdminTicketPriorityAction } = await import("./update-admin-ticket-priority");
...
describe("updateAdminTicketPriorityAction", () => {
  beforeEach(() => {
  ...
  describe("authorization ordering", () => {
    it("calls requireAdmin before creating the Data API client or the RPC", async () => {
      ...
      await updateAdminTicketPriorityAction(validFormData());
    ...
    it("an employee rejection prevents the RPC from ever being called", async () => {
      ...
      await expect(updateAdminTicketPriorityAction(validFormData())).rejects.toThrow("NEXT_REDIRECT");
    ...
    it("requireAdmin is called with no caller-provided role/id", async () => {
      ...
      await updateAdminTicketPriorityAction(validFormData({ role: "admin", userId: "attacker" }));
  ...
  describe("validation", () => {
    ...
    it("accepts a valid ticket UUID and canonical priority", async () => {
      const result = await updateAdminTicketPriorityAction(validFormData());
    ...
    it("an invalid ticket UUID prevents the RPC from being called", async () => {
      const result = await updateAdminTicketPriorityAction(validFormData({ ticketId: "not-a-uuid" }));

      expect(result.error).toBeTruthy();
      expect(rpc).not.toHaveBeenCalled();
    });

    it("an arbitrary/non-canonical priority is rejected before the RPC", async () => {
      const result = await updateAdminTicketPriorityAction(validFormData({ priority: "urgent" }));
    ...
    it("extra role/user/updatedBy/actorId form fields are read but never trusted or forwarded", async () => {
      await updateAdminTicketPriorityAction(
        validFormData({
          role: "admin",
          userId: "attacker",
          adminId: "attacker",
      ...
      expect(rpc).toHaveBeenCalledWith("admin_set_ticket_priority", {
  ...
  describe("RPC call shape", () => {
    ...
    it("calls exactly admin_set_ticket_priority with p_ticket_id/p_priority", async () => {
      await updateAdminTicketPriorityAction(validFormData({ priority: "critical" }));
  ...
  describe("revalidation only follows success", () => {
    it("a validation failure never revalidates anything", async () => {
      ...
      await updateAdminTicketPriorityAction(validFormData({ ticketId: "not-a-uuid" }));
    ...
    it("an auth failure never revalidates anything", async () => {
      ...
      await expect(updateAdminTicketPriorityAction(validFormData())).rejects.toThrow();
